 ...
 * Code inspired by and borrowed from http://www.phpied.com/image-diff/
 */
// check if there's enough input
if (empty($argv[1]) || empty($argv[2])) {
    echo 'givme at least two image filenames, please.', "\n";
...
if (!$different_pixels or ((100 * $different_pixels / $total)<=$threshold)) {
    echo "Image is the same with pixel change of,",number_format(100 * $different_pixels / $total, 2), '%';
    echo "\n";
    exit(0);
} else {
    if (empty($argv[3])) {

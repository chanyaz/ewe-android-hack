...
open class WeatherServices(endpoint: String, okHttpClient: OkHttpClient, interceptor: Interceptor, val observeOn: Scheduler, val subscribeOn: Scheduler) {
    ...
    fun getFiveDayForecast(params: WeatherForecastParams, observer: Observer<List<WeatherLocationResponse>>) {
        return weatherApi.getFiveDayForcast(params.apiKey, params.locationCode)
                .observeOn(observeOn)
                .subscribeOn(subscribeOn)
                .subscribe(observer)
    }

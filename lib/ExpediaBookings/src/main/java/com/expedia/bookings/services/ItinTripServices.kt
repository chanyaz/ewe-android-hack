...
open class ItinTripServices(endpoint: String, okHttpClient: OkHttpClient, interceptor: Interceptor, val observeOn: Scheduler, val subscribeOn: Scheduler) {
    ...
    open fun getGuestTrip(guestEmail: String, tripId: String, observer: Observer<AbstractItinDetailsResponse>): Disposable {
        return tripsApi.guestTrip(tripId, guestEmail)
                .observeOn(observeOn)
                .subscribeOn(subscribeOn)
                .subscribeObserver(observer)
    }

...
open class FlightServices(endpoint: String, okHttpClient: OkHttpClient, interceptor: Interceptor, val observeOn: Scheduler, val subscribeOn: Scheduler) {
    ...
    open fun flightSearch(params: FlightSearchParams, observer: Observer<FlightSearchResponse>): Subscription? {
        ...
        val subscription = flightApi.flightSearch(params.toQueryMap(), params.children)
                .observeOn(observeOn)
    ...
    open fun createTrip(params: FlightCreateTripParams, observer: Observer<FlightCreateTripResponse>): Subscription {
        ...
        val subscription = flightApi.createTrip(params.toQueryMap())
                                            .observeOn(observeOn)
                                            .subscribeOn(subscribeOn)
                                            .subscribe(observer)
        createTripRequestSubscription = subscription
        return subscription
    }
    ...
    open fun checkout(params: Map<String, Any>, observer: Observer<FlightCheckoutResponse>): Subscription {
        ...
        val subsciption = flightApi.checkout(params)
                                        .observeOn(observeOn)
                                        .subscribeOn(subscribeOn)
                                        .subscribe(observer)

        checkoutRequestSubscription = subsciption
        return subsciption
    }
}

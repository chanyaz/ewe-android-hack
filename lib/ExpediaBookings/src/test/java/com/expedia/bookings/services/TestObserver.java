...
public class TestObserver<T>
	...
	static String fusionModeToString(int mode) {
		switch (mode) {
		case QueueDisposable.NONE:
			return "NONE";
		case QueueDisposable.SYNC:
			return "SYNC";
		case QueueDisposable.ASYNC:
			return "ASYNC";
		default:
			return "Unknown(" + mode + ")";
		}
	...
	/**
	 * Assert that the TestObserver contains the given first and optional rest values exactly
	...
	public final void assertValuesAndClear(T expectedFirstValue, T... expectedRestValues) {
		...
		assertValueCount(n);
		assertValueAt(0, expectedFirstValue);
		for (int i = 0; i < expectedRestValues.length; i++) {
			assertValueAt(i + 1, expectedRestValues[i]);
		}
		values.clear();
	}
}

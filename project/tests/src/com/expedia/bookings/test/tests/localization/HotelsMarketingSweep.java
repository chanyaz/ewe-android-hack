...
public class HotelsMarketingSweep extends CustomActivityInstrumentationTestCase<LaunchActivity> {
	...
	public void testMethod() throws Exception {
		...
		// loop to search again with new dates at increment of 5,if there are no search results
		for (int i = 0; i < 2; i++) {
			mDriver.hotelsSearchScreen().clickOnCalendarButton();
			mDriver.delay(1);
			CalendarTouchUtils.clickNextMonth(mDriver, mDriver.hotelsSearchScreen().calendarDatePicker());
			mDriver.delay(1);
			...
			// Sort
			//check for popularity filter(no popularity string if there are no search results)
			if (mDriver.hotelsSearchScreen().sortMenu().getSortByPopularityString() != null) {

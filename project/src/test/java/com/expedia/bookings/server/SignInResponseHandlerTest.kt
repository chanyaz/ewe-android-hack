...
class SignInResponseHandlerTest {
    ...
    private fun thenCommonValuesShouldBeCorrect(user: User) {
        ...
        assertTrue(user.primaryTraveler.isLoyaltyMembershipActive)
        assertEquals(54206, user.primaryTraveler.loyaltyPointsAvailable)
        assertEquals(5601, user.primaryTraveler.loyaltyPointsPending)
        assertEquals(LoyaltyMembershipTier.TOP, user.primaryTraveler.loyaltyMembershipTier)

...
class FlightTravelerFrequentFlyerWidgetTest {
    ...
    @Test
    fun testCorrectFFNCards() {
        (widget.viewModel as FlightTravelerEntryWidgetViewModel).flightLegObservable.onNext(listOf(buildMockFlight()))
    ...
    private fun buildMockFlight() : FlightLeg {
        ...
        createFlightSegments(flight, airlineNames)
        return flight
    }

    private fun createFlightSegments(flight: FlightLeg, airlineNames: List<String>) {
        flight.segments = ArrayList<FlightSegment>()
        for (i in airlineNames.indices) {

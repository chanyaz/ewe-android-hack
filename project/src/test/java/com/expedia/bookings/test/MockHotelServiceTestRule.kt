...
class MockHotelServiceTestRule : ServicesRule<HotelServices>(HotelServices::class.java) {
    ...
    private fun getOfferResponse(responseFileName: String): HotelOffersResponse {
        ...
        val hotelSearchParams = HotelSearchParams.Builder(0).departure(suggestion).checkIn(LocalDate()).checkOut(LocalDate()).adults(1).children(emptyList()).build() as HotelSearchParams
    ...
    private fun getHotelOffersResponse(responseFileName: String): HotelOffersResponse {
        ...
        val suggestion = SuggestionV4()
        suggestion.coordinates = SuggestionV4.LatLng()
        val hotelSearchParams = HotelSearchParams.Builder(0).departure(suggestion).checkIn(LocalDate.now().plusDays(4)).checkOut(LocalDate.now().plusDays(6)).adults(2).children(listOf(2, 4)).build() as HotelSearchParams

...
public class FlightSearchPresenterTest {
	...
	public void testChangeSearchFormAfterClick() {
		...
		RecyclerView recentSearchWidgetItems = getRecentSearchList();
		recentSearchWidgetItems.getChildAt(0).performClick();
		widget.getSearchViewModel().getOriginLocationObserver().onNext(getSuggestion("DEL", "San Francisco"));
		...
		OmnitureTestUtils.assertLinkTracked("Edit Flight Search form", "App.Flight.Dest.Search.RS.Origin.Edit",
			OmnitureMatchers.withEvars(data), mockAnalyticsProvider);
		widget.getSearchViewModel().getDestinationLocationObserver().onNext(getSuggestion("SEA", "San Francisco"));
		data.clear();
		...
		widget.getSearchViewModel().getDateSelectionChanged().onNext(true);
	...
	@Test
	public void testChangeSearchFormWithNoRecentSearchSelection() {
		HashMap<Integer, String> data = new HashMap();
		setupRecentSearch();
		widget.getSearchViewModel().getOriginLocationObserver().onNext(getSuggestion("DEL", "San Francisco"));
		data.put(28, "App.Flight.Dest.Search.RS.Origin.Edit");
		OmnitureTestUtils.assertLinkNotTracked("Edit Flight Search form", "App.Flight.Dest.Search.RS.Origin.Edit",
			OmnitureMatchers.withEvars(data), mockAnalyticsProvider);
		widget.getSearchViewModel().getDestinationLocationObserver().onNext(getSuggestion("SEA", "San Francisco"));
		data.clear();
		data.put(28, "App.Flight.Dest.Search.RS.Destination.Edit");
		OmnitureTestUtils.assertLinkNotTracked("Edit Flight Search form", "App.Flight.Dest.Search.RS.Destination.Edit",
			OmnitureMatchers.withEvars(data), mockAnalyticsProvider);
		data.clear();
		widget.getSearchViewModel().setAreRecentSearchDatesInPast(false);
		widget.getSearchViewModel().getDateSelectionChanged().onNext(false);
		data.put(28, "App.Flight.Dest.Search.RS.Date.Edit");
		OmnitureTestUtils.assertLinkNotTracked("Edit Flight Search form", "App.Flight.Dest.Search.RS.Date.Edit",
			OmnitureMatchers.withEvars(data), mockAnalyticsProvider);

		data.clear();
		widget.getSearchViewModel().getDateSelectionChanged().onNext(false);
		data.put(28, "App.Flight.Dest.Search.RS.Dates.Past.Edit");
		OmnitureTestUtils.assertLinkNotTracked("Edit Flight Search form", "App.Flight.Dest.Search.RS.Dates.Past.Edit",
			OmnitureMatchers.withEvars(data), mockAnalyticsProvider);
	}

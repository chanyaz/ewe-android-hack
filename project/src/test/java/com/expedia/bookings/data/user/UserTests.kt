...
class UserTests {
    ...
    fun testVersionOneFromJSONEquality() {
        assertMemberEqualityForVersionOneUser(User(UserJSONHelper.versionOneUserJSONObject))
    }
    ...
    fun testToJSONEquality() {
        val user = User(UserJSONHelper.versionTwoUserJSONObject)
        assertEquals(UserJSONHelper.versionTwoUserJSONObject.toString(), user.toJson().toString())
    }

    @Test
    fun testToPersistentStorageJSONEquality() {
        val user = UserLoginTestUtil.mockUser(LoyaltyMembershipTier.TOP)
        val traveler = Traveler()
        traveler.firstName = "Paul"
        traveler.middleName = "Vincent"
        traveler.lastName = "Kite"

        val loyaltyInfo = UserLoyaltyMembershipInformation()
        loyaltyInfo.fromJson(UserJSONHelper.userLoyaltyMembershipInformationJSONObject)
        user.loyaltyMembershipInformation = loyaltyInfo

        user.primaryTraveler = traveler

        val expectedJSON = """{"version":2,"primaryTraveler":${Traveler().toJson()},"loyaltyMembershipInformation":${UserJSONHelper.userLoyaltyMembershipInformationJSONObject}}"""

        assertEquals(expectedJSON, user.toPersistentStorageJson().toString())
    }
    ...
    private fun assertMemberEqualityForVersionOneUser(user: User) {
        assertMemberEqualityForUser(user)

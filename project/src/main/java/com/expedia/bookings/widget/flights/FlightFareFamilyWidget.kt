...
class FlightFareFamilyWidget(context: Context, attrs: AttributeSet) : LinearLayout(context, attrs) {
    ...
    var viewModel: FlightFareFamilyViewModel by notNullAndObservable { vm ->
        ...
        vm.fareFamilyDetailsObservable.subscribe { fareDetails ->
            fareFamilyRadioGroup.removeAllViews()
            fareDetails.forEach {
                var markSelected = false
                if (vm.selectedFareFamilyObservable.value.fareFamilyCode.equals(it.fareFamilyCode)) {
                    markSelected = true
                }
                val fareFamilyItemViewModel = FareFamilyItemViewModel(context, it, vm.choosingFareFamilyObservable, markSelected, vm.roundTripObservable)
                val fareFamilyItem = LayoutInflater.from(context).inflate(R.layout.flight_fare_family_item_layout, fareFamilyRadioGroup, false) as FareFamilyItemWidget
        ...
        vm.choosingFareFamilyObservable.subscribe {
            fareFamilyRadioGroup.clearChecks()
        }
    ...
    override fun onFinishInflate() {
        ...
        if (statusBarHeight > 0) {
            var lp = fareFamilyContainer.layoutParams as android.widget.LinearLayout.LayoutParams
    ...
    fun RadioGroup.clearChecks() {
        for (i in 0..childCount - 1) {
            val v = getChildAt(i) as FareFamilyItemWidget

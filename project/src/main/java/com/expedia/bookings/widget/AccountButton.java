...
public class AccountButton extends LinearLayout {
	...
	private boolean isSignInEarnMessagingEnabled(LineOfBusiness lob) {
		return (lob == LineOfBusiness.HOTELS && PointOfSale.getPointOfSale().isEarnMessageEnabledForHotels()
			|| ((lob == LineOfBusiness.FLIGHTS || lob == LineOfBusiness.FLIGHTS_V2) && PointOfSale.getPointOfSale().isEarnMessageEnabledForFlights())
			|| lob == LineOfBusiness.PACKAGES && PointOfSale.getPointOfSale().isEarnMessageEnabledForPackages())

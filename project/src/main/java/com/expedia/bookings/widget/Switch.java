 ...
 * The user may drag the "thumb" back and forth to choose the selected option, or simply tap to toggle
 * as if it were a checkbox. The {@link #setText(CharSequence) text} property controls the text
 * displayed in the label for the switch, whereas the {@link #setTextOff(CharSequence) off} and
 * {@link #setTextOn(CharSequence) on} text controls the text on the thumb. Similarly, the
 * {@link #setTextAppearance(android.content.Context, int) textAppearance} and the related setTypeface()
...
public class Switch extends CompoundButton {
	...
	public void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		if (heightMode == MeasureSpec.EXACTLY) {
			...
			//TODO: this is sloppy
			height = mTrackDrawable.getIntrinsicHeight();
		}

		mThumbWidth = maxTextWidth + mThumbTextPadding * 2;

		mSwitchWidth = width;
		mSwitchHeight = height;

...
class HotelCheckoutSummaryWidget(context: Context, attrs: AttributeSet?, val viewModel: HotelCheckoutSummaryViewModel) : LinearLayout(context, attrs) {
    ...
    private fun resetViewsToInitialState() {
        ...
        amountDueTodayLabel.setTextSize(TypedValue.COMPLEX_UNIT_PX, resources.getDimensionPixelSize(R.dimen.amount_due_today_label_text_size).toFloat())
        totalPriceWithTaxAndFees.setTextSize(TypedValue.COMPLEX_UNIT_PX, resources.getDimensionPixelSize(R.dimen.amount_due_today_text_size).toFloat())

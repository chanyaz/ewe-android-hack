...
class HotelRoomRateView(context: Context) : LinearLayout(context) {
    ...
    var viewModel: HotelRoomRateViewModel by notNullAndObservable { vm ->
        ...
        if (isShopWithPoints || !isAirAttached && !AbacusFeatureConfigManager.isUserBucketedForTest(AbacusUtils.EBAndroidAppHotelHideStrikethroughPrice)) {
    ...
    private fun collapseRow(animationDuration: Long) {
        if (expandedMeasurementsDone) {
            ...
            collapsedContainer.setBackgroundColor(ContextCompat.getColor(context, R.color.egg_white))

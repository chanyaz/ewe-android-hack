...
class CarItinManageBookingWidgetViewModel<S>(scope: S) : ItinManageBookingWidgetViewModel() where S : HasStringProvider, S : HasWebViewLauncher, S : HasActivityLauncher, S : HasItinRepo, S : HasTripsTracking {
    ...
    init {
        val webViewScope = WebViewCardScope(scope.strings, scope.webViewLauncher, scope.itinRepo.liveDataItin.value!!, TripProducts.CAR.name, scope.tripsTracking)

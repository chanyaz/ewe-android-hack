...
class WebViewLauncher(val context: Context) : IWebViewLauncher {
    ...
    override fun launchWebViewActivity(title: Int, url: String, anchor: String?, tripId: String, scrapeTitle: Boolean) {
        val animation: Bundle = ActivityOptionsCompat.makeCustomAnimation(context, R.anim.slide_up_partially, R.anim.slide_down_partially).toBundle()
        val webViewIntent = buildWebViewIntent(title, url, anchor, tripId, scrapeTitle).intent
        (context as AppCompatActivity).startActivityForResult(webViewIntent, Constants.ITIN_WEBVIEW_REFRESH_ON_EXIT_CODE, animation)
    }

    private fun buildWebViewIntent(title: Int, url: String, anchor: String?, tripId: String, scrapeTitle: Boolean): WebViewActivity.IntentBuilder {

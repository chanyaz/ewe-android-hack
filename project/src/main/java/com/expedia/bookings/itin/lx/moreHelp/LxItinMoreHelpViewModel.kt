...
class LxItinMoreHelpViewModel<out S>(val scope: S) : IMoreHelpViewModel where S : HasStringProvider, S : HasLxRepo, S : HasLifecycleOwner, S : HasTripsTracking {
    ...
    init {
        itinLxObserver = LiveDataObserver { itin ->
            val vendorName = itin?.vendorCustomerServiceOffices?.first()?.name
            val phoneNumber = itin?.vendorCustomerServiceOffices?.first()?.phoneNumber

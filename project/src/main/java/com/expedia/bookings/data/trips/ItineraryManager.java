...
=======
    ...
    public void init(Context context) {
        ...
        userStateManager = Ui.getApplication(context).appComponent().userStateManager();
        notificationManager = Ui.getApplication(context).appComponent().notificationManager();
        tnsService = Ui.getApplication(context).flightComponent().tnsService();
    ...
    private void registerWithNewSystem() {
        ...
        if (!TextUtils.isEmpty(regId)) {
            ...
            com.expedia.bookings.data.flights.User user = new com.expedia.bookings.data.flights.User();
            user.setSiteid(siteId);
            user.setTuid((int) userTuid);

            String appName = ProductFlavorFeatureConfiguration.getInstance().getAppNameForMobiataPushNameHeader();

            List<com.expedia.bookings.data.flights.Flight> flights = new ArrayList<>();
            ...
            for (Flight flight : actualFlights) {
                com.expedia.bookings.data.flights.Flight flightToAdd = new com.expedia.bookings.data.flights.Flight();
                flightToAdd.setDepartureDate(JodaUtils.format(flight.getOriginWaypoint().getBestSearchDateTime().toLocalDateTime().toDateTime(), "yyyy-MM-dd HH:mm:ss"));
                flightToAdd.setArrivalDate(JodaUtils.format(flight.getDestinationWaypoint().getBestSearchDateTime().toLocalDateTime().toDateTime(), "yyyy-MM-dd HH:mm:ss"));
                flightToAdd.setFlightNo(flight.getPrimaryFlightCode().mNumber);
                flightToAdd.setDestination(flight.getDestinationWaypoint().mAirportCode);
                flightToAdd.setOrigin(flight.getOriginWaypoint().mAirportCode);
                flightToAdd.setAirline(flight.getPrimaryFlightCode().mAirlineCode);
            ...
            tnsService.registerForFlights(appName, user, flights, regId, new rx.Observer<TNSRegisterDeviceResponse>() {
                @Override
                ...
                public void onNext(TNSRegisterDeviceResponse tnsRegisterDeviceResponse) {
                    Log.d(LOGGING_TAG, "registerForPushNotifications response:" + (tnsRegisterDeviceResponse == null ? "null" : tnsRegisterDeviceResponse.getStatus()));
                }
            });
        }
    }

        ...
        viewModel.navigationSubject.subscribe {
            when (it.first) {
                ...
                LineOfBusiness.FLIGHTS -> {
                    if (PointOfSale.getPointOfSale().supports(LineOfBusiness.FLIGHTS) && isShowFlightsBrowserExperience(context)) {
                        openFlightsViaSoftPrompt()
                    } else if ((PointOfSale.getPointOfSale().supports(LineOfBusiness.FLIGHTS)
                            && PointOfSale.getPointOfSale().pointOfSaleId != PointOfSaleId.INDIA) || isShowFlightsCheckoutWebview(context)) {

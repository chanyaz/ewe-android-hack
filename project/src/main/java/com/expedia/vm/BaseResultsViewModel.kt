...
abstract class BaseResultsViewModel(context: Context) {
    ...
    fun shouldShowRichContent(context: Context): Boolean = (isRichContentEnabled(context) && getLineOfBusiness() == LineOfBusiness.FLIGHTS_V2) ||
            (isRichContentForPackagesEnabled(context) && getLineOfBusiness() == LineOfBusiness.PACKAGES)
}

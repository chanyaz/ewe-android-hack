...
class BundleOverviewViewModel(val context: Context, val packageServices: PackageServices?) {
    ...
    init {
        hotelParamsObservable.subscribe { params ->
            ...
            searchPackageSubscriber = packageServices?.packageSearch(params, if (isMidAPIEnabled(context)) ProductSearchType.MultiItemHotels else ProductSearchType.OldPackageSearch)?.subscribeObserver(makeResultsObserver(PackageSearchType.HOTEL, params.isChangePackageSearch()))
        }
        ...
        return object : DisposableObserver<BundleSearchResponse>() {
            override fun onNext(response: BundleSearchResponse) {
                if (response.hasErrors()) {
                    errorObservable.onNext(getApiCallFailingDetails(type, isChangeSearch, response.firstError))

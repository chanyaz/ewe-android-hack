...
public class FlightsSearchResultsSingleCellSteps {
	...
	private void legalComplianceClick(int resid, String text, boolean outBound) {
		onView(allOf(withId(resid), (outBound ? isDescendantOfA(withId(R.id.widget_flight_outbound)) : isDescendantOfA(withId(R.id.widget_flight_inbound))))).check(matches(withText(containsString(text)))).perform(click());
		Common.delay(2);
		Espresso.pressBack();
	}
	...
	@And("^Validate legal compliance link on flight overview screen is clickable and isOutbound : (true|false)$")
	public void validatelegalcomplaianceoutboundoverview(boolean outBound) throws Throwable {
		...
		legalComplianceClick(R.id.show_payment_fees, "Airline fee applies based on payment method", outBound);

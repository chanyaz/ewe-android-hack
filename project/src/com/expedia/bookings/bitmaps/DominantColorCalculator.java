...
public class DominantColorCalculator {
    ...
    private int findTertiaryAccentColor(final MedianCutQuantizer.ColorNode primary, final MedianCutQuantizer.ColorNode secondary) {
        ...
        for (MedianCutQuantizer.ColorNode color : mWeightedPalette) {
            if (ColorAvgUtils.calculateContrast(color, primary) >= TERTIARY_MIN_CONTRAST_PRIMARY
                    && ColorAvgUtils.calculateContrast(color, secondary) >= TERTIARY_MIN_CONTRAST_SECONDARY) {
                return color.getRgb();
            }
        }
        ...
        // by 45%
        return ColorAvgUtils.changeBrightness(secondary.getRgb(), 0.45f);
    }
    ...
    private int findPrimaryTextColor(final MedianCutQuantizer.ColorNode primary) {
        ...
        for (MedianCutQuantizer.ColorNode color : mPalette) {
            if (ColorAvgUtils.calculateContrast(color, primary) >= PRIMARY_TEXT_MIN_CONTRAST) {
                return color.getRgb();
            }
        }
        ...
        // brightness
        return ColorAvgUtils.calculateYiqLuma(primary.getRgb()) >= 128 ? Color.BLACK : Color.WHITE;
    }
    ...
    private static float calculateWeight(MedianCutQuantizer.ColorNode node, final float maxCount) {
        return FloatUtils.weightedAverage(
			ColorAvgUtils.calculateColorfulness(node), 2f,
